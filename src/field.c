...
void renderGeneration(char field[][WIDTH_Y_80]) {
    ...
    for (int x = 0; x < HEIGHT_X_25; x++) {
        printw("#");
        for (int y = 0; y < WIDTH_Y_80; y++) { printw("%c", field[x][y]); }
        printw("#\n");
    }
    printw("##################################################################################\n");
    printw("Control:\n");
...
void aliveOrDead(char field[][WIDTH_Y_80], char field_next_generation[][WIDTH_Y_80]) {
    for (int i = 0; i < HEIGHT_X_25; ++i) {
        for (int j = 0; j < WIDTH_Y_80; ++j) {
            ...
            if (field[i][j] == ALIVE && (counter > 3 || counter < 2)) { field_next_generation[i][j] = DEAD; }
            if (field[i][j] == DEAD && counter == 3) { field_next_generation[i][j] = ALIVE; }
        }
    }
}
...
void updateField(char field[][WIDTH_Y_80], char field_next_generation[][WIDTH_Y_80]) {
    for (int i = 0; i < HEIGHT_X_25; ++i) {
        for (int j = 0; j < WIDTH_Y_80; ++j) { field[i][j] = field_next_generation[i][j]; }
    }
}
